...
# -*- coding: utf-8 -*-
"""
FENAdmin - Gestor de Usuarios y Grupos v1.3 (corregido definitivo)
Creador: @fenreitsu
Lógica de tipos de cuenta basada en account_type.py
"""
...
# ============================================================================
# CONSTANTES DE GRUPOS (según account_type.py)
# ============================================================================
DESKTOP_GROUPS = {
    "adm", "dialout", "fax", "cdrom", "floppy", "tape", "audio", "dip",
    "video", "plugdev", "scanner", "users"
}
ADMIN_GROUPS = DESKTOP_GROUPS | {"sudo", "lpadmin", "sambashare"}
ADMIN_SPECIFIC_GROUPS = ADMIN_GROUPS - DESKTOP_GROUPS  # {'sudo', 'lpadmin', 'sambashare'}
IGNORED_GROUPS = {"netdev"}  # No influyen en el tipo de cuenta

PRIVILEGE_TO_GROUP = {
    "adm": "adm", "dialout": "dialout", "fax": "fax", "cdrom": "cdrom",
    "floppy": "floppy", "tape": "tape", "audio": "audio", "dip": "dip",
    "video": "video", "plugdev": "plugdev", "netdev": "netdev",
    "scanner": "scanner", "lpadmin": "lpadmin", "sambashare": "sambashare"
}

# ============================================================================
# CONFIGURACIÓN DE LOGOS Y TEMAS (igual que antes)
# ============================================================================
...
THEMES = {
    "dark": {
        ...
        "accent": "#58a6ff", "accent2": "#1f6feb", "success": "#3fb950",
        "warning": "#d29922", "danger": "#f85149", "info": "#79c0ff",
        "text": "#e6edf3", "text2": "#8b949e", "text3": "#484f58",
        "select_bg": "#1f6feb", "select_fg": "#ffffff", "header_bg": "#161b22",
        "row_odd": "#0d1117", "row_even": "#161b22", "button_bg": "#21262d",
        "button_fg": "#e6edf3", "button_hover": "#30363d", "entry_bg": "#0d1117",
        "entry_fg": "#e6edf3", "label_fg": "#e6edf3", "disabled_fg": "#484f58",
        "tag_sys": "#161b22", "tag_sys_fg": "#8b949e", "scrollbar": "#30363d",
        "terminal_bg": "#0a0e12", "terminal_fg": "#00ff00", "terminal_error": "#ff4444",
    },
    "light": {
        "bg": "#f6f8fa", "bg2": "#ffffff", "bg3": "#eaeef2", "border": "#d0d7de",
        "accent": "#0969da", "accent2": "#218bff", "success": "#1a7f37",
        "warning": "#9a6700", "danger": "#cf222e", "info": "#0550ae",
        "text": "#1f2328", "text2": "#656d76", "text3": "#afb8c1",
        "select_bg": "#0969da", "select_fg": "#ffffff", "header_bg": "#f6f8fa",
        "row_odd": "#ffffff", "row_even": "#f6f8fa", "button_bg": "#f6f8fa",
        "button_fg": "#1f2328", "button_hover": "#eaeef2", "entry_bg": "#ffffff",
        "entry_fg": "#1f2328", "label_fg": "#1f2328", "disabled_fg": "#afb8c1",
        "tag_sys": "#eaeef2", "tag_sys_fg": "#656d76", "scrollbar": "#d0d7de",
        "terminal_bg": "#f0f0f0", "terminal_fg": "#006600", "terminal_error": "#cc0000",
    }
}
...
def validate_password(password):
    ...
    weak_patterns = ['password', '123456', 'qwerty', 'abc123', 'admin', 'root', 'toor']
    if password.lower() in weak_patterns:
        return False, "Contraseña demasiado común."
    if re.search(r'(.)\1{3,}', password):
        return False, "Evite secuencias repetitivas."
    return True, "OK"
...
def generate_password(length=16, use_special=True):
    ...
    password = [secrets.choice(uppercase), secrets.choice(lowercase), secrets.choice(digits)]
    if use_special:
...
def get_all_groups():
    ...
    for g in grp.getgrall():
        groups.append({
            ...
            "passwd": g.gr_passwd, "system": g.gr_gid < 1000,
        })
    ...
    for p in pwd.getpwall():
        ...
        try:
            ...
            if out and 'L' in out.split()[1] if len(out.split()) > 1 else False:
                locked = True
        users.append({
            "name": p.pw_name, "uid": p.pw_uid, "gid": p.pw_gid,
            "gecos": p.pw_gecos, "home": p.pw_dir, "shell": p.pw_shell,
            "system": is_system, "locked": locked,
        })
...
# ============================================================================
# TERMINAL INTERACTIVA (sin cambios, igual que antes)
# ============================================================================
class InteractiveTerminal:
    def __init__(self, parent, theme, fire_warning):
        ...
        self.output_area = scrolledtext.ScrolledText(
            self.frame, height=12, bg=theme["terminal_bg"], fg=theme["terminal_fg"],
        ...
        self.prompt_label = tk.Label(input_frame, text="$> ", font=('Courier New', 10, 'bold'),
                                     fg=theme["accent"], bg=theme["bg"])
        self.prompt_label.pack(side=tk.LEFT)
        self.command_entry = tk.Entry(input_frame, font=('Courier New', 10), bg=theme["entry_bg"],
                                      fg=theme["entry_fg"], insertbackground=theme["accent"], relief=tk.FLAT)
        self.command_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, ipady=4)
        self.command_entry.bind('<Return>', self.execute_command)
        self.command_entry.bind('<Up>', self.history_up)
        self.command_entry.bind('<Down>', self.history_down)
        self.exec_btn = tk.Button(input_frame, text="▶ Ejecutar", font=('Courier New', 9, 'bold'),
                                  bg=theme["accent2"], fg="#fff", relief=tk.FLAT, cursor="hand2",
                                  command=lambda: self.execute_command())
        self.exec_btn.pack(side=tk.RIGHT, padx=(5, 0))
        self.clear_btn = tk.Button(input_frame, text="🗑 Limpiar", font=('Courier New', 9),
                                   bg=theme["button_bg"], fg=theme["button_fg"], relief=tk.FLAT,
                                   cursor="hand2", command=self.clear_output)
        self.clear_btn.pack(side=tk.RIGHT, padx=(5, 0))
        self.current_directory = os.getcwd()
        self._write_output("=" * 60, "info")
        self._write_output("  FENAdmin Terminal Interactiva v1.3", "success")
    ...
    def execute_command(self, event=None):
        ...
        dangerous_patterns = [
            "userdel nobody", "userdel 'nobody'", 'userdel "nobody"',
            "groupdel nogroup", "groupdel 'nogroup'", 'groupdel "nogroup"',
            "usermod nobody", "usermod 'nobody'", 'usermod "nobody"',
            "groupmod nogroup", "groupmod 'nogroup'", 'groupmod "nogroup"',
            "passwd nobody", "passwd 'nobody'", 'passwd "nobody"',
            "chage nobody", "chage 'nobody'", 'chage "nobody"',
        ]
        ...
        def run():
            try:
                ...
                self._write_output("ERROR: Tiempo límite excedido (30s)", "error")
            except Exception as e:
                self._write_output(f"ERROR: {str(e)}", "error")
            finally:
...
# ============================================================================
# CLASE PRINCIPAL FENAdmin (CORREGIDA)
# ============================================================================
class FENAdmin(tk.Tk):
    def __init__(self):
        ...
        self.title("FENAdmin v1.3 - by @fenreitsu")
    ...
    def _build_ui(self):
        ...
        self.lbl_logo = tk.Label(logo_frame, text="FENAdmin v1.3", font=("Courier New", 16, "bold"))
        self.lbl_logo.pack(side="left")
        self._load_logo_image()
        self.lbl_creator = tk.Label(self.topbar, text="by @fenreitsu", font=("Courier New", 11), padx=4)
        self.lbl_creator.pack(side="left", pady=6)
        right_frame = tk.Frame(self.topbar)
        right_frame.pack(side="right", padx=12)
        self.btn_theme = tk.Button(right_frame, text="☀  Claro", font=("Courier New", 11, "bold"),
                                   relief="flat", cursor="hand2", padx=10, pady=4, command=self._toggle_theme)
        self.btn_theme.pack(side="right", padx=4)
        root_text = "● ROOT" if is_root() else "● USER"
        root_color = T["success"] if is_root() else T["warning"]
        self.lbl_root = tk.Label(right_frame, text=root_text, font=("Courier New", 11, "bold"),
        ...
        self.status_label = tk.Label(self.statusbar, text="  FENAdmin listo. Usa la terminal inferior para comandos.",
                                     font=("Courier New", 10), anchor="w")
        self.status_label.pack(side="left", fill="x", expand=True, padx=6)
        self.lbl_time = tk.Label(self.statusbar, font=("Courier New", 10), padx=8)
        self.lbl_time.pack(side="right")
        self._update_clock()

    def _build_sidebar(self):
        T = self.T
        tk.Label(self.sidebar, text="NAVEGACIÓN", font=("Courier New", 10, "bold"), anchor="w", padx=14, pady=12).pack(fill="x")
        self.nav_buttons = {}
        nav_items = [("users", "👤  Usuarios", self._show_users), ("groups", "👥  Grupos", self._show_groups)]
        for key, label, cmd in nav_items:
            btn = tk.Button(self.sidebar, text=label, font=("Courier New", 12), anchor="w",
                            relief="flat", cursor="hand2", padx=14, pady=8, command=cmd)
            btn.pack(fill="x")
            self.nav_buttons[key] = btn
        tk.Frame(self.sidebar, height=1).pack(fill="x", pady=8, padx=14)
        tk.Label(self.sidebar, text="FILTROS", font=("Courier New", 10, "bold"), anchor="w", padx=14, pady=4).pack(fill="x")
        self.chk_system = tk.Checkbutton(self.sidebar, text="  Mostrar sistema", font=("Courier New", 11),
                                         variable=self.show_system, anchor="w", relief="flat", cursor="hand2",
                                         command=self._apply_filter)
        self.chk_system.pack(fill="x", padx=8)
        tk.Frame(self.sidebar, height=1).pack(fill="x", pady=8, padx=14)
        self.btn_refresh = tk.Button(self.sidebar, text="↻  Actualizar", font=("Courier New", 11), anchor="w",
                                     relief="flat", cursor="hand2", padx=14, pady=6, command=self._full_refresh)
        self.btn_refresh.pack(fill="x")
        tk.Frame(self.sidebar, height=20).pack(fill="x", expand=True)
        tk.Label(self.sidebar, text="💡 TIP", font=("Courier New", 10, "bold"), anchor="w", padx=14, pady=4).pack(fill="x")
        tip_text = tk.Label(self.sidebar, text="Usa la terminal\ninferior para\ncomandos directos",
                            font=("Courier New", 10), anchor="w", justify="left", padx=14)
    ...
    def _build_users_view(self, parent):
        ...
        tk.Label(top_toolbar, text="Gestión de Usuarios", font=("Courier New", 16, "bold"), anchor="w", padx=16, pady=8).pack(side="left")
        actions = [("😁 Nuevo Usuario", self._dlg_new_user, "accent"), ("🪪 Editar Usuario", self._dlg_edit_user, "warning"),
                   ("🗑 Eliminar", self._delete_user, "danger"), ("🔒 Bloquear", self._lock_user, "warning"),
        ...
        tk.Label(filter_bar, text="🔍 Buscar:", font=("Courier New", 11, "bold"), bg=T["bg"], fg=T["text"]).pack(side="left", padx=(15, 5))
        self.search_entry = tk.Entry(filter_bar, font=("Courier New", 12), relief="flat", bg=T["entry_bg"],
                                     fg=T["entry_fg"], insertbackground=T["accent"], width=25)
        self.search_entry.pack(side="left", padx=(0, 15), ipady=3)
        self.search_entry.bind("<KeyRelease>", lambda e: self._apply_user_filters())
        clear_btn = tk.Button(filter_bar, text="✕ Limpiar filtros", font=("Courier New", 10), relief="flat",
                              cursor="hand2", command=self._clear_user_filters)
        ...
        self.toggle_btn = tk.Button(filter_header, text="▼ FILTROS AVANZADOS", font=("Courier New", 10, "bold"),
        ...
        cols = ("Usuario", "UID", "GID", "Home", "Shell", "Estado", "Tipo")
        self.tree_users = ttk.Treeview(table_frame, columns=cols, show="headings", selectmode="browse")
        col_widths = [140, 70, 70, 240, 160, 100, 120]
        ...
        self.tree_users.bind("<Button-1>", lambda e: self.ctx_user.unpost())
        info_bar = tk.Frame(parent, height=28)
        info_bar.pack(fill="x", side="bottom")
        info_bar.pack_propagate(False)
        self.lbl_user_count = tk.Label(info_bar, text="", font=("Courier New", 10), anchor="w", padx=10)
    ...
    def _build_groups_view(self, parent):
        ...
        tk.Label(toolbar, text="Gestión de Grupos", font=("Courier New", 16, "bold"), anchor="w", padx=16, pady=8).pack(side="left")
        actions = [("＋ Nuevo Grupo", self._dlg_new_group, "accent"), ("🗑 Eliminar", self._delete_group, "danger"),
        ...
        tk.Label(search_frame, text="🔍 Buscar grupo:", font=("Courier New", 11, "bold"), bg=T["bg"], fg=T["text"]).pack(side="left", padx=(15, 5))
        self.group_search_entry = tk.Entry(search_frame, font=("Courier New", 12), relief="flat", bg=T["entry_bg"],
                                           fg=T["entry_fg"], insertbackground=T["accent"], width=25)
        ...
        self.tree_groups = ttk.Treeview(table_frame, columns=cols, show="headings", selectmode="browse")
        ...
        self.tree_groups.bind("<Button-1>", lambda e: self.ctx_group.unpost())
        info_bar = tk.Frame(parent, height=28)
        info_bar.pack(fill="x", side="bottom")
        info_bar.pack_propagate(False)
        self.lbl_group_count = tk.Label(info_bar, text="", font=("Courier New", 10), anchor="w", padx=10)
        self.lbl_group_count.pack(side="left")

    def _create_advanced_filters(self):
        T = self.T
        self.filter_vars = {
            "username": tk.StringVar(), "uid_min": tk.StringVar(), "uid_max": tk.StringVar(),
            "shell": tk.StringVar(), "state": tk.StringVar(), "gecos": tk.StringVar(),
            "home": tk.StringVar(), "account_type": tk.StringVar(),
        }
        filters_frame = tk.Frame(self.advanced_filters, pady=8)
        filters_frame.pack(fill="x", padx=15)
        row1 = tk.Frame(filters_frame)
        row1.pack(fill="x", pady=3)
        tk.Label(row1, text="Usuario:", font=("Courier New", 10), width=12, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        username_entry = tk.Entry(row1, textvariable=self.filter_vars["username"], width=20, font=("Courier New", 10), relief="flat")
        username_entry.pack(side="left", padx=(0, 20))
        username_entry.bind("<KeyRelease>", lambda e: self._apply_user_filters())
        tk.Label(row1, text="UID desde:", font=("Courier New", 10), width=10, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        uid_min_entry = tk.Entry(row1, textvariable=self.filter_vars["uid_min"], width=8, font=("Courier New", 10), relief="flat")
        uid_min_entry.pack(side="left", padx=(0, 5))
        uid_min_entry.bind("<KeyRelease>", lambda e: self._apply_user_filters())
        tk.Label(row1, text="hasta:", font=("Courier New", 10), width=5, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        uid_max_entry = tk.Entry(row1, textvariable=self.filter_vars["uid_max"], width=8, font=("Courier New", 10), relief="flat")
        ...
        tk.Label(row2, text="Shell:", font=("Courier New", 10), width=12, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        shell_combo = ttk.Combobox(row2, textvariable=self.filter_vars["shell"], width=25, font=("Courier New", 10), state="readonly")
        ...
        tk.Label(row2, text="Estado:", font=("Courier New", 10), width=10, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        state_combo = ttk.Combobox(row2, textvariable=self.filter_vars["state"], width=15, font=("Courier New", 10), state="readonly")
        ...
        tk.Label(row2, text="Tipo:", font=("Courier New", 10), width=8, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        type_combo = ttk.Combobox(row2, textvariable=self.filter_vars["account_type"], width=15, font=("Courier New", 10), state="readonly")
        ...
        tk.Label(row3, text="GECOS:", font=("Courier New", 10), width=12, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        gecos_entry = tk.Entry(row3, textvariable=self.filter_vars["gecos"], width=52, font=("Courier New", 10), relief="flat")
        ...
        tk.Label(row4, text="Home:", font=("Courier New", 10), width=12, anchor="e", bg=T["bg"], fg=T["text"]).pack(side="left", padx=(0, 5))
        home_entry = tk.Entry(row4, textvariable=self.filter_vars["home"], width=52, font=("Courier New", 10), relief="flat")
    ...
    def _apply_user_filters(self):
        ...
        for u in users:
            if not show_sys and u["system"]:
                continue
            if search_text:
                if (search_text not in u["name"].lower() and search_text not in u["gecos"].lower()
                    and search_text not in u["home"].lower() and search_text not in u["shell"].lower()):
                    continue
            ...
            self.tree_users.insert("", "end", iid=u["name"],
                                   values=(u["name"], u["uid"], u["gid"], u["home"], u["shell"], state, account_type),
                                   tags=(tag,))
            count += 1
        T = self.T
    ...
    # -------------------------------------------------------------------------
    # NUEVOS MÉTODOS DE LÓGICA (basados en account_type.py)
    # -------------------------------------------------------------------------
    def _determine_account_type(self, username):
        grupos = set(get_user_groups(username))
        grupos_relevantes = grupos - IGNORED_GROUPS
        if ADMIN_GROUPS.issubset(grupos_relevantes):
            return "Admin"
        elif DESKTOP_GROUPS.issubset(grupos_relevantes) and not (grupos_relevantes & ADMIN_SPECIFIC_GROUPS):
            return "Desktop User"
        else:
            return "Personalizado"

    def _set_account_type_groups(self, username, account_type, extra_groups=None):
        """Aplica la plantilla de grupos (Admin o Desktop). Nunca elimina grupos extra."""
        if account_type == "Admin":
            grupos_destino = ADMIN_GROUPS.copy()
            grupos_a_eliminar = set()
        elif account_type == "Desktop User":
            grupos_destino = DESKTOP_GROUPS.copy()
            grupos_a_eliminar = ADMIN_SPECIFIC_GROUPS.copy()
        else:
            return True
        if extra_groups:
            grupos_destino.update(extra_groups)
        grupos_actuales = set(get_user_groups(username))
        grupos_a_anadir = grupos_destino - grupos_actuales
        grupos_a_eliminar = grupos_a_eliminar & grupos_actuales
        for g in grupos_a_anadir:
            run_cmd(f"usermod -aG '{g}' '{username}'")
        for g in grupos_a_eliminar:
            run_cmd(f"gpasswd -d '{username}' '{g}'")
        return True

    def _apply_special_privileges(self, username, selected_privileges):
        """Sincroniza SOLO los grupos correspondientes a los switches.
           Esta función SOLO debe usarse cuando el tipo es Personalizado."""
        grupos_actuales = set(get_user_groups(username))
        grupos_deseados = set(selected_privileges)
        privilegios_validos = set(PRIVILEGE_TO_GROUP.values())
        for g in grupos_deseados - grupos_actuales:
            run_cmd(f"usermod -aG '{g}' '{username}'")
        for g in (grupos_actuales - grupos_deseados) & privilegios_validos:
            run_cmd(f"gpasswd -d '{username}' '{g}'")

    # -------------------------------------------------------------------------
    # MÉTODOS DE LA GUI (corregidos)
    # -------------------------------------------------------------------------
    ...
    def _apply_theme(self):
        ...
        style.configure("Treeview", background=T["bg2"], foreground=T["text"], fieldbackground=T["bg2"],
                        rowheight=26, font=("Courier New", 11))
        style.configure("Treeview.Heading", background=T["bg3"], foreground=T["text"], font=("Courier New", 11, "bold"), relief="flat")
        style.map("Treeview", background=[("selected", T["select_bg"])], foreground=[("selected", T["select_fg"])])
        style.configure("Vertical.TScrollbar", background=T["bg3"], troughcolor=T["bg2"], arrowcolor=T["text2"], relief="flat")
        style.configure("Horizontal.TScrollbar", background=T["bg3"], troughcolor=T["bg2"], arrowcolor=T["text2"], relief="flat")
    ...
    def _theme_all(self, widget):
        ...
        try:
            if cls == "Frame":
                ...
                widget.configure(bg=T["button_bg"], fg=T["button_fg"], activebackground=T["button_hover"],
                                 activeforeground=T["text"], highlightbackground=T["border"], highlightcolor=T["accent"])
            elif cls == "Entry":
                widget.configure(bg=T["entry_bg"], fg=T["entry_fg"], insertbackground=T["accent"],
                                 highlightbackground=T["border"], highlightcolor=T["accent"], highlightthickness=1)
            elif cls == "Checkbutton":
                widget.configure(bg=T["bg"], fg=T["text"], selectcolor=T["accent"], activebackground=T["bg"],
    ...
    def _styled_btn(self, parent, text, cmd, style="accent"):
        T = self.T
        colors = {"accent": (T["accent2"], "#fff"), "danger": (T["danger"], "#fff"),
                  "warning": (T["warning"], "#fff"), "success": (T["success"], "#fff"),
                  "default": (T["button_bg"], T["button_fg"])}
        bg, fg = colors.get(style, colors["default"])
        btn = tk.Button(parent, text=text, font=("Courier New", 11, "bold"), bg=bg, fg=fg,
                        activebackground=T["button_hover"], activeforeground=T["text"],
                        relief="flat", cursor="hand2", padx=10, pady=4, command=cmd)
        return btn
    ...
    def refresh_groups(self):
        ...
        for g in groups:
            ...
            self.tree_groups.insert("", "end", iid=g["name"], values=(g["name"], g["gid"], members, tipo, has_passwd),
    ...
    # -------------------------------------------------------------------------
    # DIALOGO NUEVO USUARIO (CORREGIDO)
    # -------------------------------------------------------------------------
    def _dlg_new_user(self):
        ...
        if dlg.result:
            ...
            cmd = f"useradd"
            gecos_str = f",{d.get('work_addr','')},{d.get('work_phone','')},{d.get('personal_phone','')}".replace("'","")
            if any([d.get('work_addr'), d.get('work_phone'), d.get('personal_phone')]):
                cmd += f" -c '{gecos_str}'"
            ...
            if d.get("account_type") != "Admin":
                ...
                cmd_aging = ""
            if d.get("expires"):
                cmd = cmd.replace(f"-e '{expire_date}'", f"-e '{d['expires']}'") if 'expire_date' in locals() else cmd + f" -e '{d['expires']}'"
            ...
            if out:
                self._log_to_terminal(out, "info")
            if err and rc != 0:
                self._log_to_terminal(err, "error")
            if rc == 0:
                pw_cmd = f"echo '{d['username']}:{d['password']}' | chpasswd"
                run_cmd(pw_cmd)
                if d.get("account_type") != "Admin" and cmd_aging:
                    run_cmd(cmd_aging)
                account_type = d.get("account_type", "Personalizado")
                extra_groups = [g.strip() for g in d.get("groups","").split(",") if g.strip()]

                # Aplicar plantilla si es Admin o Desktop
                if account_type in ("Admin", "Desktop User"):
                    self._set_account_type_groups(d['username'], account_type, extra_groups)
                # Aplicar privilegios especiales solo si es Personalizado
                if account_type == "Personalizado" and hasattr(dlg, 'permisos_seleccionados'):
                    self._apply_special_privileges(d['username'], dlg.permisos_seleccionados)

                self._log_to_terminal(f"✅ Usuario '{d['username']}' creado", "success")
                self.refresh_users()
            else:
    ...
    # -------------------------------------------------------------------------
    # DIALOGO EDITAR USUARIO (CORREGIDO)
    # -------------------------------------------------------------------------
    def _dlg_edit_user(self):
        ...
        try:
            ...
            additional_groups = [g for g in all_groups if g != username and g != primary_group_name]
            gecos_parts = (p.pw_gecos or "").split(",")
            current = {
                "username": p.pw_name, "work_addr": gecos_parts[1].strip() if len(gecos_parts) > 1 else "",
                "work_phone": gecos_parts[2].strip() if len(gecos_parts) > 2 else "",
                "personal_phone": gecos_parts[3].strip() if len(gecos_parts) > 3 else "",
                "home": p.pw_dir, "shell": p.pw_shell, "uid": p.pw_uid, "gid": p.pw_gid,
                "group": primary_group_name, "groups": ",".join(additional_groups),
                "privileges": privileges, "account_type": account_type_current,
            }
        except KeyError:
        ...
        current_permissions = {}
        for grupo in PRIVILEGE_TO_GROUP.values():
        ...
        dlg.saved_permissions = current_permissions
        self.wait_window(dlg)
        if not dlg.result:
            return
        d = dlg.result
        ...
        # Aplicar cambios básicos (usermod)
        cmd_parts = ["usermod"]
        has_changes = False
        new_gecos = f",{d.get('work_addr','')},{d.get('work_phone','')},{d.get('personal_phone','')}"
        old_gecos = f",{current.get('work_addr','')},{current.get('work_phone','')},{current.get('personal_phone','')}"
        if new_gecos != old_gecos:
            safe_gecos = new_gecos.replace("'", "")
            cmd_parts.append(f"-c '{safe_gecos}'")
            has_changes = True
        ...
        if d.get("password"):
            ...
            self._log_to_terminal("✅ Contraseña actualizada", "success")

        # LÓGICA DE TIPO DE CUENTA Y PRIVILEGIOS
        account_type = d.get("account_type", "Personalizado")
        extra_groups = [g.strip() for g in d.get("groups","").split(",") if g.strip()]

        if account_type in ("Admin", "Desktop User"):
            # Aplicar plantilla completa (no usar switches)
            self._set_account_type_groups(username, account_type, extra_groups)
            # No aplicar _apply_special_privileges porque ya se han establecido todos los grupos necesarios
        elif account_type == "Personalizado":
            # Solo aplicar los switches (permisos especiales)
            if hasattr(dlg, 'permisos_seleccionados'):
                self._apply_special_privileges(username, dlg.permisos_seleccionados)

        # Ajustar expiración según tipo
        if account_type == "Admin":
            run_cmd(f"chage -E -1 '{username}'")
            run_cmd(f"chage -M -1 '{username}'")
            run_cmd(f"chage -I -1 '{username}'")
            self._log_to_terminal(f"✅ Admin '{username}': sin expiración", "success")
        else:
            expire_date = (datetime.datetime.now() + datetime.timedelta(days=90)).strftime("%Y-%m-%d")
            run_cmd(f"chage -E '{expire_date}' '{username}'")
            run_cmd(f"chage -M 90 '{username}'")
            run_cmd(f"chage -I 30 '{username}'")
            self._log_to_terminal(f"⚠ '{username}' expirará el {expire_date}", "warning")
    ...
    # =========================================================================
    # MÉTODOS FALTANTES (eliminación, bloqueo, grupos, context menus)
    # =========================================================================
    def _delete_user(self):
        ...
        tk.Label(dialog, text=f"Eliminar Usuario\n¿Eliminar el usuario '{username}'?\n\n"
                 ...
                 font=("Courier New", 13, "bold"), bg=self.T["bg"], fg=self.T["text"]).pack(pady=(15, 0))
        ...
        tk.Button(btn_frame, text="🗑 SÍ A TODO", font=("Courier New", 11, "bold"), bg=self.T["danger"], fg="white",
                  padx=15, pady=5, command=lambda: responder("full")).pack(side="left", padx=5)
        tk.Button(btn_frame, text="✓ Solo usuario", font=("Courier New", 11), bg=self.T["warning"], fg="white",
                  padx=15, pady=5, command=lambda: responder("user_only")).pack(side="left", padx=5)
        tk.Button(btn_frame, text="✗ Cancelar", font=("Courier New", 11), bg=self.T["disabled_fg"], fg=self.T["text"],
                  padx=15, pady=5, command=lambda: responder("cancel")).pack(side="left", padx=5)
    ...
    def _unlock_user(self):
        ...
        if rc == 0:
            ...
            respuesta = messagebox.askyesno("Contraseña requerida", f"El usuario '{username}' no tiene una contraseña válida.\n\n"
    ...
    def _context_menu_user(self, event):
        ...
        self.ctx_user.configure(bg=T["bg2"], fg=T["text"], activebackground=T["accent"], activeforeground="#fff",
                                font=("Courier New", 11))
    ...
    def _change_password(self):
        ...
        if dlg.result:
            ...
            if not is_root():
                ...
                messagebox.showwarning("Sin privilegios", "Se requiere root.")
                return
            self._log_to_terminal(f"Ejecutando: $ passwd {username}", "command")
            out, err, rc = run_cmd(cmd)
            if rc == 0:
    ...
    def _show_user_groups(self):
        ...
        messagebox.showinfo(f"Grupos de '{username}'", f"El usuario '{username}' pertenece a:\n\n" + "\n".join(f"  • {g}" for g in groups))
    ...
    def _dlg_edit_group(self):
        ...
        try:
            ...
            current = {"name": g.gr_name, "gid": g.gr_gid, "members": ", ".join(g.gr_mem), "passwd": g.gr_passwd}
        except KeyError:
        ...
        if dlg.result:
            ...
            if d.get("new_name") and d["new_name"] != gname:
                cmds.append(f"groupmod -n '{d['new_name']}' '{gname}'")
            if d.get("gid") and str(d["gid"]) != str(current["gid"]):
                cmds.append(f"groupmod -g '{d['gid']}' '{gname}'")
            if d.get("password") and d["password"] != current.get("passwd", ""):
                cmds.append(f"# gpasswd '{gname}' (contraseña: {d['password']})")
            if d.get("members") is not None:
                ...
                for m in new_members - old_members:
                    cmds.append(f"usermod -aG '{gname}' '{m}'")
                for m in old_members - new_members:
                    cmds.append(f"gpasswd -d '{m}' '{gname}'")
            if not is_root():
    ...
    def _delete_group(self):
        ...
        try:
            g = grp.getgrnam(gname)
            if g.gr_gid < 1000:
                if not messagebox.askyesno("Advertencia", f"'{gname}' es un grupo de sistema. ¿Estás seguro de eliminarlo?\nEsto podría afectar el funcionamiento del sistema."):
                    return
        except:
            pass
        if not messagebox.askyesno("Eliminar Grupo", f"¿Eliminar el grupo '{gname}'?\nEsta acción no se puede deshacer."):
            return
    ...
    def _context_menu_group(self, event):
        ...
        self.ctx_group.configure(bg=T["bg2"], fg=T["text"], activebackground=T["accent"], activeforeground="#fff",
                                 font=("Courier New", 11))
    ...
    def _show_group_members(self):
        ...
        try:
            ...
            messagebox.showinfo(f"Miembros de '{gname}'", f"Grupo: {gname}\nGID: {g.gr_gid}\n\nMiembros:\n{members}")
        except KeyError:
            messagebox.showerror("Error", f"Grupo '{gname}' no encontrado.")

# ============================================================================
# CLASES DE DIÁLOGOS (AdvancedUserDialog, GroupDialog, PasswordDialog)
# ============================================================================
# (Estas clases son exactamente iguales a las que proporcionaste, no hay cambios)
# Para ahorrar espacio, se incluyen a continuación pero sin modificaciones.
# ============================================================================
class AdvancedUserDialog(tk.Toplevel):
    ...
    def _build(self, data):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text=self.title(), font=("Courier New", 15, "bold"),
        ...
        fields = [
            ...
            ("Dirección laboral",      "work_addr"),
            ("Teléfono de trabajo",    "work_phone"),
        ...
        for label, key in fields:
            ...
            e = tk.Entry(row, font=("Courier New", 12),
        ...
        current_shell = data.get("shell", "")
        self.shell_var = tk.StringVar(value=current_shell)
        shell_row = tk.Frame(form, bg=T["bg"])
        shell_row.pack(fill="x", **pad)
        tk.Label(shell_row, text="Shell", font=("Courier New", 11),
                 bg=T["bg"], fg=T["text2"], width=25, anchor="w").pack(side="left")
        shell_values = ([current_shell] + [s for s in shells if s != current_shell]) if current_shell else shells
        shell_combo = ttk.Combobox(shell_row, textvariable=self.shell_var, values=shell_values, width=33)
        shell_combo.configure(font=("Courier New", 11))
        if current_shell:
        ...
        row = tk.Frame(form, bg=T["bg"])
        row.pack(fill="x", **pad)
        tk.Label(row, text="Tipo de cuenta", font=("Courier New", 11),
                 bg=T["bg"], fg=T["text2"], width=25, anchor="w").pack(side="left")
        ...
        for i, (atype, desc) in enumerate(account_types):
            ...
            tk.Label(btn_row_at, text=f"  {desc}", font=("Courier New", 9),
        ...
        row = tk.Frame(form, bg=T["bg"])
        row.pack(fill="x", **pad)
        tk.Label(row, text="Contraseña", font=("Courier New", 11),
                 bg=T["bg"], fg=T["text2"], width=25, anchor="w").pack(side="left")

        pw_frame = tk.Frame(row, bg=T["bg"])
        pw_frame.pack(side="left")

        self.password_entry = tk.Entry(pw_frame, font=("Courier New", 12), show="*",
                                       bg=T["entry_bg"], fg=T["entry_fg"],
                                       insertbackground=T["accent"],
                                       highlightbackground=T["border"],
                                       highlightcolor=T["accent"],
        ...
        self.gen_btn = tk.Button(pw_frame, text="🔑 Generar", font=("Courier New", 10),
        ...
        self.toggle_btn = tk.Button(pw_frame, text="👁", font=("Courier New", 10),
        ...
        tk.Label(req_frame, text="Requisitos de contraseña:", font=("Courier New", 10, "bold"),
        ...
        for req in requirements:
            tk.Label(req_frame, text=req, font=("Courier New", 9),
        ...
        # Campos básicos de la pestaña avanzada
        campos_frame = tk.Frame(main_adv_frame, bg=T["bg"])
        campos_frame.pack(fill="x", pady=(0, 10))

        pad_adv = dict(padx=14, pady=3)

        # UID
        row = tk.Frame(campos_frame, bg=T["bg"])
        row.pack(fill="x", **pad_adv)
        tk.Label(row, text="UID (dejar vacío para auto):", font=("Courier New", 11),
                bg=T["bg"], fg=T["text2"], width=30, anchor="w").pack(anchor="w")
        self.uid_entry = tk.Entry(row, font=("Courier New", 12),
                                bg=T["entry_bg"], fg=T["entry_fg"],
                                insertbackground=T["accent"],
                                highlightbackground=T["border"],
                                highlightcolor=T["accent"],
        ...
        def _make_autocomplete_combo(parent, width=28):
            var = tk.StringVar()
            combo = ttk.Combobox(parent, textvariable=var, values=all_system_groups, width=width)
            combo.configure(font=("Courier New", 12))
            def _on_key(event):
                typed = var.get()
                if typed:
                    filtered = [g for g in all_system_groups if g.lower().startswith(typed.lower())]
        ...
        row = tk.Frame(campos_frame, bg=T["bg"])
        row.pack(fill="x", **pad_adv)
        tk.Label(row, text="Grupo principal:", font=("Courier New", 11),
                bg=T["bg"], fg=T["text2"], width=30, anchor="w").pack(anchor="w")
        ...
        self._gp_listbox = tk.Listbox(row, font=("Courier New", 11),
                                      bg=T["entry_bg"], fg=T["entry_fg"],
        ...
        tk.Button(gp_input_row, text="✓ Establecer", font=("Courier New", 10),
                  bg=T["accent2"], fg="#fff", relief="flat", cursor="hand2",
                  padx=6, command=_set_gp).pack(side="left", padx=(4, 2))
        tk.Button(gp_input_row, text="✕ Limpiar", font=("Courier New", 10),
        ...
        row = tk.Frame(campos_frame, bg=T["bg"])
        row.pack(fill="x", **pad_adv)
        tk.Label(row, text="Grupos adicionales:", font=("Courier New", 11),
                bg=T["bg"], fg=T["text2"], width=30, anchor="w").pack(anchor="w")
        ...
        self._ga_listbox = tk.Listbox(row, font=("Courier New", 11),
                                      bg=T["entry_bg"], fg=T["entry_fg"],
                                      selectbackground=T["accent2"],
        ...
        tk.Button(ga_input_row, text="＋ Agregar", font=("Courier New", 10),
                  bg=T["accent2"], fg="#fff", relief="flat", cursor="hand2",
                  padx=6, command=_add_ga).pack(side="left", padx=(4, 2))
        tk.Button(ga_input_row, text="✕ Eliminar", font=("Courier New", 10),
                  bg=T["button_bg"], fg=T["text2"], relief="flat", cursor="hand2",
        ...
        row = tk.Frame(campos_frame, bg=T["bg"])
        row.pack(fill="x", **pad_adv)
        tk.Label(row, text="Fecha expiración (YYYY-MM-DD o never):", font=("Courier New", 11),
                bg=T["bg"], fg=T["text2"], width=30, anchor="w").pack(anchor="w")
        self.expires_entry = tk.Entry(row, font=("Courier New", 12),
                                    bg=T["entry_bg"], fg=T["entry_fg"],
                                    insertbackground=T["accent"],
                                    highlightbackground=T["border"],
                                    highlightcolor=T["accent"],
        ...
        row = tk.Frame(campos_frame, bg=T["bg"])
        row.pack(fill="x", **pad_adv)
        tk.Label(row, text="Días de inactividad (-1 para nunca):", font=("Courier New", 11),
                bg=T["bg"], fg=T["text2"], width=30, anchor="w").pack(anchor="w")
        self.inactive_entry = tk.Entry(row, font=("Courier New", 12),
                                    bg=T["entry_bg"], fg=T["entry_fg"],
                                    insertbackground=T["accent"],
                                    highlightbackground=T["border"],
                                    highlightcolor=T["accent"],
        ...
        for i, (grupo, desc) in enumerate(permisos):
            ...
            switch, update_func = create_switch(frame, var)
            switch.pack(side="left")
            if not hasattr(self, 'switches'):
                self.switches = {}
            self.switches[grupo] = update_func
            lbl = tk.Label(frame, text=desc, font=("Courier New", 11),
                        bg=T["bg"], fg=T["text"], cursor="hand2")
            lbl.pack(side="left", padx=10)
            def toggle_permission(event=None, v=var, upd=update_func):
                v.set(not v.get())
                upd(True)
            lbl.bind("<Button-1>", toggle_permission)

        # Función para mostrar/ocultar switches según tipo de cuenta
        def on_account_type_change(*args):
            if hasattr(self, 'permisos_switches_frame'):
                if self.account_type.get() == "Personalizado":
                    ...
                    # Cargar permisos guardados al mostrar
        ...
        tk.Button(
            btn_row, text="Cancelar", font=("Courier New", 11),
        ...
        tk.Button(
            btn_row, text=action, font=("Courier New", 11, "bold"),
            bg=T["accent2"], fg="#fff",
            relief="flat", cursor="hand2", padx=14, pady=5,
            command=self._submit
        ).pack(side="right", padx=4)

        # Inicializar estado de los switches según saved_permissions
        self._load_saved_permissions()
            ...
            for grupo, activo in self.saved_permissions.items():
                if grupo in self.perm_vars:
                    ...
                    if hasattr(self, 'switches') and grupo in self.switches:
                        # Ahora self.switches[grupo] es la función update_switch
                        self.switches[grupo](with_animation=False)
    ...
    def _submit(self):
        ...
        d["shell"] = self.shell_var.get().strip()
...
class GroupDialog(tk.Toplevel):
    ...
    def _build(self, data):
        ...
        tk.Label(mb_frame, text="Miembros:", font=("Courier New", 11),
        ...
        self._mb_combo.configure(font=("Courier New", 12))
        ...
        self._mb_listbox = tk.Listbox(mb_frame, font=("Courier New", 11),
                                      bg=T["entry_bg"], fg=T["entry_fg"],
                                      selectbackground=T["accent2"],
                                      height=4, width=30, relief="flat",
                                      highlightbackground=T["border"],
        ...
        tk.Button(mb_input_row, text="＋ Agregar", font=("Courier New", 10),
                  bg=T["accent2"], fg="#fff", relief="flat", cursor="hand2",
                  padx=6, command=_add_mb).pack(side="left", padx=(4, 2))
        tk.Button(mb_input_row, text="✕ Eliminar", font=("Courier New", 10),
                  bg=T["button_bg"], fg=T["text2"], relief="flat", cursor="hand2",
...
class PasswordDialog(tk.Toplevel):
    ...
    def _build(self, username):
        ...
        tk.Label(hdr, text=f"🔑  Contraseña para: {username}",
                 font=("Courier New", 13, "bold"), bg=T["bg2"], fg=T["accent"], padx=16).pack(anchor="w")
        tk.Frame(self, height=1, bg=T["border"]).pack(fill="x")
        ...
        for label, key in [("Nueva contraseña", "pw1"), ("Confirmar contraseña", "pw2")]:
            ...
            e = tk.Entry(frame, show="*", font=("Courier New", 12),
                         bg=T["entry_bg"], fg=T["entry_fg"],
                         insertbackground=T["accent"],
                         highlightbackground=T["border"],
                         highlightcolor=T["accent"],
            ...
            if key == "pw1":
                self.toggle_btn = tk.Button(frame, text="👁", font=("Courier New", 10),
                                           bg=T["button_bg"], fg=T["text"], relief="flat", cursor="hand2",
                                           command=self._toggle_visibility)
                self.toggle_btn.pack(side="left", padx=2)
                self.gen_btn = tk.Button(frame, text="🔑 Generar", font=("Courier New", 10),
                                        bg=T["accent2"], fg="#fff", relief="flat", cursor="hand2",
        ...
        tk.Label(req_frame, text="Requisitos:", font=("Courier New", 10, "bold"),
                bg=T["bg"], fg=T["text"]).pack(anchor="w")
        requirements = [
            "• Mínimo 8 caracteres",
        ...
        btn_row = tk.Frame(self, bg=T["bg"], pady=10)
        btn_row.pack(fill="x", padx=14)
        tk.Button(btn_row, text="Cancelar", font=("Courier New", 11),
                  bg=T["button_bg"], fg=T["text2"], relief="flat", cursor="hand2",
                  padx=14, pady=5, command=self.destroy).pack(side="right", padx=4)
        tk.Button(btn_row, text="Cambiar contraseña", font=("Courier New", 11, "bold"),
                  bg=T["accent2"], fg="#fff", relief="flat", cursor="hand2",
...
if __name__ == "__main__":
    if not is_root():
        ...
        print("   Lanzando en modo LECTURA. Usa: sudo python3 fenadmin_corregido.py\n")
    app = FENAdmin()
    app.mainloop()
